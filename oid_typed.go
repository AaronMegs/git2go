...
// toC converts an input oid without mutating it. For the Go zero value, it
// returns a typed-SHA1 copy so shared/map-key Oids remain stable.
...
// outC returns writable storage for libgit2 output parameters. Callers must
// keep the Oid alive until the C call returns.
