...
// TestTransactionSetReflog covers Transaction.SetReflog, which replaces the
// reflog that will be committed together with a locked reference update.
func TestTransactionSetReflog(t *testing.T) {
	repo := createTestRepo(t)
	defer cleanupTestRepo(t, repo)
	...
	// Give the branch its own reflog with a known single entry, then hand that
	// reflog to the transaction so the committed reflog is the one we built.
	commit, err := repo.LookupCommit(commitID)
	checkFatal(t, err)
	defer commit.Free()

	branch, err := repo.CreateBranch("tx-reflog", commit, false)
	checkFatal(t, err)
	defer branch.Free()

	const refName = "refs/heads/tx-reflog"

	reflog, err := repo.ReadReflog(refName)
	checkFatal(t, err)
	defer reflog.Free()

	if err := reflog.Append(commitID, sig, "transaction reflog entry"); err != nil {
		t.Fatalf("Append failed: %v", err)
	}
	...
	// libgit2 requires a target on a locked ref before the transaction commits.
	if err := tx.SetTarget(refName, commitID, sig, "transaction target"); err != nil {
		t.Fatalf("SetTarget failed: %v", err)
	}
	if err := tx.SetReflog(refName, reflog); err != nil {
		t.Fatalf("SetReflog failed: %v", err)
	}
	if err := tx.Commit(); err != nil {
		t.Fatalf("Commit failed: %v", err)
	}

	// The reflog handed to the transaction must be the one that was persisted.
	persisted, err := repo.ReadReflog(refName)
	checkFatal(t, err)
	defer persisted.Free()

	found := false
	for i := 0; i < persisted.EntryCount(); i++ {
		if persisted.EntryByIndex(i).Message() == "transaction reflog entry" {
			found = true
			break
		}
	}
	if !found {
		t.Fatal("SetReflog did not persist the supplied reflog entry")
	}
}

func TestTransactionSetReflogRejectsNilReflog(t *testing.T) {
	repo := createTestRepo(t)
	defer cleanupTestRepo(t, repo)

	reflogSeedCommit(t, repo)

	tx, err := repo.NewTransaction()
	checkFatal(t, err)
	defer tx.Free()

	if err := tx.SetReflog("refs/heads/whatever", nil); !IsErrorCode(err, ErrorCodeInvalid) {
		t.Fatalf("SetReflog(nil) error = %v, want ErrorCodeInvalid", err)
	}
}
