...
- `ObjectIdType`, `ObjectIdSHA1`, `ObjectIdSHA256`, `Oid.Type()` and
  `Oid.Bytes()`.
- `InitRepositoryWithOidType`, unified `RepositoryInitOptions` with both
  `OidType` and `RefdbType`, and `Repository.OidType`.
- Reftable repository initialization, runtime capability/format detection,
  explicit files/reftable backend constructors and backend compaction.
- Complete Go-defined refdb backend bridge, including optional
  init/compress/lock/unlock capabilities, per-instance iterators and transaction
  lock payloads.
- Reference transaction and complete reflog entry read/write APIs.
...
- SHA1/SHA256 × files/reftable four-way coverage including commit creation,
  branch updates, close/reopen persistence and repository config assertions.
...
- Custom refdb backend ownership and callback safety: backend/output wrappers are
  consumed after C ownership transfer, Free is idempotent, callback panics become
  libgit2 user errors, and iterator/lock handles are released exactly once.
- `Oid{}` is normalized to typed SHA1 before C calls, including OID shortening.
- Repository init validates object/ref storage types and NUL bytes before disk
  side effects, and cleans up partially returned repositories on failure.
- Merge-base array results are disposed and empty/nil input is rejected safely.
- Transactions reject embedded NULs and become closed after a successful commit,
  preventing reuse of released backend lock payloads.
- Repository-derived refdb backends retain their repository owner until ownership
