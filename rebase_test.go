...
func TestRebaseAbort(t *testing.T) {
	...
	// Capture the initial branch name before setupRepoForRebase switches HEAD to
	// branchName; defaultBranchName() reports the *current* HEAD branch.
	baseBranch := defaultBranchName(t, repo)
	...
	rebase, err := performRebaseOnto(repo, baseBranch, nil)
	checkFatal(t, err)
	defer rebase.Free()
...
func TestRebaseGpgSigned(t *testing.T) {
	...
	// Rebase onto the base branch
	rebase, err := performRebaseOnto(repo, baseBranch, &rebaseOpts)
	checkFatal(t, err)
	defer rebase.Free()
