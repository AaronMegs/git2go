...
> **历史文档说明（2026-09-07）**
>
> 本文记录的是 `feat-reftable` 分支整合进 `feat-sha256-reftable` 之前的状态，保留
> 作为设计与审计依据。以下内容在整合后已发生变化：
>
> - **构建标签极性已反转**。reftable 绑定现在**默认编入**，不再需要
>   `libgit2_reftable` tag；链接不含 reftable 的 libgit2 时改用
>   `libgit2_no_reftable` tag 退化为 files-only。
> - `GIT2GO_HAS_REFDB_BACKEND_INIT` 条件编译已移除（pin 的基线始终提供该回调）。
> - 临时 `OidType` 枚举已由统一的 `ObjectIdType` 取代。
>
> 当前状态以 `docs/sha256-reftable-integration-report.md` 为准。
...
8. ✅ **vendor 与重构均已完成**：main `939362a3c`（commit `8b4a398`）+ v36 typed `Oid` + promoted ABI guard；latest-main static/dynamic 与四组合已恢复并通过。
