...
// TestFilesTransactionMultipleRefsAtomic pins down the property that makes
// libgit2's transaction API incompatible with reftable: several references can
// be locked simultaneously and then committed together.
//
// The `files` backend supports this because each lock is a separate
// `refs/…/x.lock` file. reftable cannot, because its atomicity unit is a single
// "addition" holding one lock on the whole reference database — a second
// concurrent addition fails with a lock error. If this test ever has to change
// shape to keep passing, the transaction contract itself has changed.
// See docs/reftable-transaction-research.md.
func TestFilesTransactionMultipleRefsAtomic(t *testing.T) {
	repo := createTestRepo(t)
	defer cleanupTestRepo(t, repo)
	commitID := seedCommit(t, repo)
	...
	checkFatal(t, err)
	if format != RefdbFiles {
		t.Skipf("test asserts files-backend behaviour, got %v", format)
	}
	...
	names := []string{
		"refs/heads/tx-multi-a",
		"refs/heads/tx-multi-b",
		"refs/heads/tx-multi-c",
	}
	...
	// Hold locks on every reference at the same time. This is the step that
	// reftable's single whole-database lock cannot express.
	for _, name := range names {
		if err := tx.LockRef(name); err != nil {
			t.Fatalf("LockRef(%s) failed: %v", name, err)
		}
	}
	for _, name := range names {
		if err := tx.SetTarget(name, commitID, sig, "atomic multi-ref update"); err != nil {
			t.Fatalf("SetTarget(%s) failed: %v", name, err)
		}
	}

	// Nothing is visible before Commit.
	for _, name := range names {
		if _, err := repo.References.Lookup(name); err == nil {
			t.Fatalf("%s exists before Commit; the update was not staged", name)
		}
	}
	...
	// After Commit every reference must point at the staged target.
	for _, name := range names {
		ref, err := repo.References.Lookup(name)
		if err != nil {
			t.Fatalf("Lookup(%s) after Commit failed: %v", name, err)
		}
		if !ref.Target().Equal(commitID) {
			t.Errorf("%s target = %v, want %v", name, ref.Target(), commitID)
		}
		ref.Free()
	}
}
